...
void loop() {
  ...
  // If obstacle detected between 2 and 26 cm, stop and evaluate further
    ...
    // Turn the servo right to check the right side first
    myServo.write(0);
    ...
    if (rightDistance > 26 || rightDistance == 0) {
      ...
      // If something is detected on the right, turn the servo left to check the left side
      myServo.write(180);
      delay(1000);  // Give time for servo to move
      ...
      // If something is detected on both sides, go backward
        ...
        backwards(100);
        delay(500);
        makeUTurn(175);  // Move backward 
      }
    }

    // After checking right and left, return the servo to 90 degrees (center)
